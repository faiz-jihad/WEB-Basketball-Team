...
export const AccountPage: React.FC = () => {
  const { fan, bookedTickets, addToast, language, firebaseUser } = useAppStore();
      ...
      <section className="min-h-screen pt-32 pb-24 px-6 bg-brand-black flex flex-col items-center justify-center relative overflow-hidden">
        ...
        >
          ...
          <button
            ...
            className="w-full p-4 bg-brand-orange hover:bg-brand-burnt text-brand-black rounded-xl font-display font-black tracking-[0.2em] uppercase transition-all flex items-center justify-center gap-3 cursor-pointer"
          >
            <UserPlus size={18} />
            Login with Google
          </button>
          ...
          <div className={`flex-1 text-center ${isRtl ? 'lg:text-right' : 'lg:text-left'} w-full`}>
            ...
            <div className="bg-black/40 rounded-2xl p-6 border border-white/5 w-full">
              <div className="flex justify-between items-end mb-3 font-display">
                ...
                <span className="text-brand-orange text-lg font-black">{fan.xp} <span className="text-xs text-gray-500 font-bold">/ {nextLevelThreshold} XP</span></span>
              </div>
                  ...
                  <div key={idx} className="relative bg-white/5 hover:bg-white/10 transition-colors border border-white/10 rounded-2xl p-6 overflow-hidden group flex items-center justify-between gap-4">
                    ...
                    <div className="relative z-10 flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] text-gray-400 font-display font-bold uppercase">{new Date(ticket.date).toLocaleDateString()}</span>
                      </div>
